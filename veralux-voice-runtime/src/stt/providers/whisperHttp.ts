...
function buildWhisperUrl(whisperUrl: string, language?: string, prompt?: string): string {
  let url = whisperUrl;
  const sep = (u: string) => (u.includes('?') ? '&' : '?');
  if (language) {
    url = `${url}${sep(url)}language=${encodeURIComponent(language)}`;
  }
  if (prompt && prompt.trim()) {
    url = `${url}${sep(url)}prompt=${encodeURIComponent(prompt.trim())}`;
  }
  return url;
}
...
export class WhisperHttpProvider implements STTProvider {
  ...
  public async transcribe(audio: STTAudioInput, opts: STTOptions = {}): Promise<STTTranscript> {
    ...
    const whisperUrl = buildWhisperUrl(baseUrl, opts.language, opts.prompt);
