...
export class SessionManager {
  ...
  public getTransport(callControlId: CallSessionId): TransportSession | undefined {
  ...
  public teardown(callControlId: CallSessionId, reason?: string, context: SessionLogContext = {}): void {
    ...
    if (env.CALL_TRANSCRIPT_DIR && transcript.turns.length > 0) {
      ...
      void fs.promises
        ...
        .then(() => fs.promises.writeFile(filePath, JSON.stringify(transcript, null, 2), 'utf8'))
        .then(() => {
          log.info(
            { event: 'call_transcript_written', file_path: filePath, call_control_id: transcript.callControlId },
            'call transcript written',
          );
        })
        .catch((err) => {
          log.warn(
            { err, file_path: filePath, call_control_id: transcript.callControlId },
            'call transcript write failed',
          );
        });
    }
  }
   ...
   */
  public getVoiceModeInfo(callControlId: CallSessionId): {
    mode: 'preset' | 'cloned';
    available: boolean;
   ...
   */
  public isVoiceCloningAvailable(callControlId: CallSessionId): boolean {
    const session = this.sessions.get(callControlId);
    if (!session) {
      return false;
    }
