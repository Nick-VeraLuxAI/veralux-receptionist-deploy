...
export class CallAudioCoordinator {
  ...
  /** Feeds the preroll ring in STT processing order (called from ChunkedSTT). Do not push from onInboundFrame. */
  public pushFrameForPreRoll(buffer: Buffer, sampleRateHz: number): void {
    if (!buffer || buffer.length < 2) return;
    const samples = buffer.length >> 1;
    if (samples === 0) return;
    const pcm16 = new Int16Array(buffer.buffer, buffer.byteOffset, samples);
    this.ringBuffer.push(pcm16, sampleRateHz);
  }
