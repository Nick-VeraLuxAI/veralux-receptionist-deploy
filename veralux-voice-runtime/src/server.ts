...
function logTtsBytesReady(
  ...
  source: 'kokoro' | 'coqui_xtts' = 'kokoro',
): void {
...
async function ensureGreetingAsset(overrideText?: string): Promise<void> {
  ...
  try {
    ...
    // audio when PLAYBACK_PROFILE=hd but the call is PSTN and expects 8k/16k).
    const pipelineApplied = true;
    {
...
const GREETING_RETRY_MS = 60_000;

function scheduleGreetingWithRetry(): void {
  void (async () => {
    await ensureGreetingAsset();
    try {
      await fs.promises.stat(path.join(env.AUDIO_STORAGE_DIR, 'greeting.wav'));
      return;
    } catch {
      log.info({ retryInMs: GREETING_RETRY_MS }, 'greeting asset missing, will retry');
      setTimeout(scheduleGreetingWithRetry, GREETING_RETRY_MS);
    }
  })();
}
...
export function buildServer(): { app: express.Express; server: http.Server; sessionManager: SessionManager } {
  ...
  if (env.STT_DEBUG_DIR) {
    fs.mkdirSync(env.STT_DEBUG_DIR, { recursive: true });
    log.debug({ dir: env.STT_DEBUG_DIR }, 'STT debug dir ensured');
  }
  ...
  // ==================== Call Management API (Voice Mode Hot-Swap) ====================
  ...
  app.get('/v1/calls/:callControlId/voice', (req, res) => {
    ...
    res.json({
      callControlId,
      voiceMode: info.mode,
      voiceCloningAvailable: info.available,
      clonedVoiceLabel: info.clonedVoiceLabel,
    });
  });

  /**
   * POST /v1/calls/:callControlId/voice
  ...
  app.post('/v1/calls/:callControlId/voice', (req, res) => {
    ...
    // Optional speakerWavUrl for one-time cloned voice override
    const speakerWavUrl =
      typeof body?.speakerWavUrl === 'string' && body.speakerWavUrl.trim()
        ? body.speakerWavUrl.trim()
        : undefined;

    // Attempt to set voice mode
    const success = sessionManager.setVoiceMode(callControlId, mode, speakerWavUrl);
    ...
    log.info(
      {
        event: 'voice_mode_api_set',
        call_control_id: callControlId,
        mode,
        has_speaker_wav_url: !!speakerWavUrl,
      },
      'voice mode set via API',
    );

    // Return updated voice mode info
    const info = sessionManager.getVoiceModeInfo(callControlId);
    res.json({
      callControlId,
      voiceMode: info?.mode ?? mode,
      voiceCloningAvailable: info?.available ?? false,
      clonedVoiceLabel: info?.clonedVoiceLabel,
      updated: true,
    });
  });
