...
- tts: discriminated by `mode`. **Kokoro**: `mode: "kokoro_http"`, `kokoroUrl`, optional `voice`, `format`, `sampleRate`. **Coqui XTTS**: `mode: "coqui_xtts"`, `coquiXttsUrl`, optional `voice`, `speakerWavUrl`, `language`, `format`, `sampleRate`.
...
To use Coqui XTTS instead of Kokoro, set `tts` to:
```json
  "tts": {
    "mode": "coqui_xtts",
    "coquiXttsUrl": "http://localhost:7002/api/tts",
    "voice": "optional_speaker_name",
    "speakerWavUrl": "optional_wav_url_for_voice_cloning",
    "language": "en"
  }
```
...
- `tts` selects TTS backend: Kokoro (`kokoro_http`) or Coqui XTTS (`coqui_xtts`), and overrides URL, voice, format, and sample rate per mode.
