#!/usr/bin/env bash
# Start backing services (Postgres), run migrations, start the Node server, and open the owner dashboard.
# Usage: ./scripts/run_services_and_owner_dashboard.sh
# Optional: ADMIN_API_KEY=yourkey ./scripts/run_services_and_owner_dashboard.sh
...
PORT="${PORT:-4000}"
DASHBOARD_URL="http://127.0.0.1:${PORT}/owner"
...
echo "[run] Opening owner dashboard in 3s: $DASHBOARD_URL"
