  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 700px; margin: 0 auto; color: #333;">
      <div style="background: #1a1a2e; padding: 24px 32px; border-radius: 8px 8px 0 0;">
        <h1 style="color: #C8A951; margin: 0; font-size: 22px;">${companyName}</h1>
      ...
      <div style="border: 1px solid #e0e0e0; border-top: none; padding: 24px 32px; border-radius: 0 0 8px 8px;">
        ...
        <table style="width: 100%; border-collapse: collapse; margin: 20px 0; font-size: 14px;">
          <thead>
            <tr style="background: #f8f8f8;">
              <th style="padding: 10px 12px; text-align: left; border-bottom: 2px solid #C8A951;">Item</th>
              <th style="padding: 10px 12px; text-align: left; border-bottom: 2px solid #C8A951;">Type</th>
              <th style="padding: 10px 12px; text-align: center; border-bottom: 2px solid #C8A951;">Qty</th>
              <th style="padding: 10px 12px; text-align: right; border-bottom: 2px solid #C8A951;">Unit Price</th>
              <th style="padding: 10px 12px; text-align: right; border-bottom: 2px solid #C8A951;">Total</th>
            </tr>
          </thead>
          ...
          <tfoot>
            ...
            <tr style="background: #f8f8f8;">
              ...
              <td style="padding: 10px 12px; text-align: right; font-weight: 700; font-size: 16px; color: #C8A951;">$${quoteData.grandTotal.toFixed(2)}</td>
            </tr>
          </tfoot>
        </table>
        ...
        <p style="color: #999; font-size: 12px; margin-top: 16px;">Generated by ${companyName}</p>
      </div>
    </div>
  `;
  return sendMail(to, `Quote ${quoteData.quoteNumber} from ${companyName}`, html);
}
...
export async function sendQuoteNotificationEmail(
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h2 style="color: #C8A951;">New Quote Generated</h2>
      ...
      <table style="font-size: 14px; margin: 16px 0;">
        ...
        <tr><td style="padding: 4px 12px 4px 0; font-weight: 600;">Total:</td><td style="font-weight: 700; color: #C8A951;">$${quoteData.grandTotal.toFixed(2)}</td></tr>
      </table>
      <p><strong>Items:</strong></p>
      <ul style="font-size: 14px;">${itemList}</ul>
      <p style="color: #666; font-size: 13px;">Log in to the owner panel to review, edit, or send this quote.</p>
    </div>
  `;
